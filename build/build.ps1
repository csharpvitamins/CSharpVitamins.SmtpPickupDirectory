cd ..\CSharpVitamins.SmtpPickupDirectory\
...
if ($semver) {
    ...
    & nuget.exe pack -version $semver -prop Configuration=Release -build -verbosity detailed -symbol -outputdirectory ..\build\
...
if ($packaged) {
    ...
    if ($decision -eq 0) {
        ...
        & nuget.exe push ..\build\CSharpVitamins.SmtpPickupDirectory.$semver.*.nupkg
